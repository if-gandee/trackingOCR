...
class UE_SceneRendering(SceneRendering):
    ...
    DEPTH_CHANNEL_NAME = 'FinalImageSceneDepth.R'
    ...
    def get_depth_image(self, dtype=None) -> np.ndarray:
        ''' get depth image from exr file, in floating point format
        '''
        if self.DEPTH_CHANNEL_NAME not in self.m_exr_image.channel_names:
            return None
        
        output = self.m_exr_image.get_channel(self.DEPTH_CHANNEL_NAME)
