...
class ExifInfo:
    ...
    def init_from_image_file(imgfilename):
        ...
        ex = {PIL.ExifTags.TAGS[key]:val for key, val in ex.items() if key in PIL.ExifTags.TAGS}
        obj.datetime_original = datetime.datetime.strptime(ex['DateTimeOriginal'],'%Y:%m:%d %H:%M:%S')
...
def poisson_blend(img, mask, chg_ij, chg_vals, 
                  outdict = None, w_fix = 1.0,
                  add_connection = None,
    ...
    for ich in range(img.shape[-1]):
        ...
        if formulate_only:
            ...
            if solver_cache is not None:
                ...
                assert (solver_cache.A - A).nnz == 0, 'solver cache has a different Ax=b'
...
def find_contours(mask):
    ...
    import cv2
    mask = mask > 0
    res = cv2.findContours(mask.astype(np.uint8)*255, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
    pathlist = []
    for p in res[1]:
...
def distance_transform(mask):
    ...
    '''
    import cv2
    dmat = cv2.distanceTransform(mask.astype(np.uint8), cv2.DIST_L2, cv2.DIST_MASK_PRECISE)
...
def multi_band_blending(img_src, mask_src, img_dst, n_band, sigma = None):
    ...
    '''
    import cv2
    assert np.allclose(img_src.shape, img_dst.shape), 'src and dst image must be of the same shape'
    assert np.allclose(img_src.shape[:2], mask_src.shape), 'src and mask must be of the same dimension'
...
def unclip_and_resize_image(img, mask_original, img_original = None, use_original_size = True):
    ...
    if img_original is None:
        if np.ndim(img) == 3:
            img_original = np.zeros([*mask_original.shape, img.shape[-1]], dtype=img.dtype)
        else:
