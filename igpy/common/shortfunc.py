...
def get_components_from_labelmap(lbmap : np.ndarray, 
    ...
    res = ndimg.labeled_comprehension(index_map, lbmap, target_labels, add_pixel_indices, int, -1)
    return label2pix
...
def colorize_labelmap(lbmap : np.ndarray, 
                      colormap_name : str = 'distinct') -> np.ndarray:
...
def get_colormap(name, n, distinct_color_attempts = 50) -> np.ndarray:
    ...
    if name == 'distinct':
        import distinctipy
        cc = distinctipy.get_colors(n, n_attempts=distinct_color_attempts)
...
def listdir(dirname, name_pattern = None, 
    ...
    str_prepend = dirname + '/' if (dirname[-1] != '/' and dirname[-1] !='\\') else dirname
