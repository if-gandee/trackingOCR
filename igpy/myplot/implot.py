...
import cv2
import numpy as np
from ..common import shortfunc as sf
...
def draw_polyline(img : np.ndarray, pts : np.ndarray, line_width : int = 1 , 
                  color3f = None, is_closed = False, return_copy = False):
...
def draw_rectangle(img : np.ndarray, xywh : np.ndarray, line_width : int = 1,
                   color3f = None, return_copy = False):
...
def draw_mask_overlay(img:np.ndarray, mask:np.ndarray):
    ...
    elif output.dtype == np.float or output.dtype == np.float32:
...
def draw_calibration_grid(img, pts, pattern_size_wh):
    ...
    '''
    canvas: np.ndarray = sf.to_uint8(img)
    if canvas.ndim == 2: # gray scale image, convert to rgb
        canvas = np.dstack([canvas]*3)
    output = cv2.drawChessboardCorners(canvas, tuple(pattern_size_wh), np.reshape(pts,(-1,1,2)), True)
    return output
...
def draw_keypoints(img, cv_keypoints : list, color3f = None, return_copy = False):
    ...
    '''
    canvas = sf.to_uint8(img)
    if color3f is None:
        color = (-1,-1,-1)
    else:
        color = np.atleast_1d(color3f)*255
        color = tuple(color.astype(int))
    cv2.drawKeypoints(canvas, cv_keypoints, canvas, color, cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    return canvas
    
def draw_markers(img, pts_xy, color3f = None, 
                 scale = 1.0, marker_type = '+',
                 return_copy = False, line_width = 1.0):
    ...
    mkcolor = (np.array(color3f) * 255).astype(np.int).tolist()
